...
export function createSelectorCreator(id, memoize, ...memoizeOptions) {
    return (...funcs) => {
        ...
        const selector = (state, props, ...args) => {
            const params = dependencies.map(dependency => dependency(state, props, ...args));
        ...
        selector.id = id;
        window.SelectorRegistry.push(selector);
...
export function createStructuredSelector(selectors, selectorCreator = createSelector) {
    ...
    return selectorCreator('structured', objectKeys.map(key => selectors[key]), (...values) => {
