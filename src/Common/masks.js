...
export const addSubstitutionInformation = substitution => {
    ...
    return {
        ...substitution,
        substitutionInfo: substitution.SUBJECT_ID_SUBSTITUTING
            ? substitution.isOld
                ? 'instead-by'
                : 'instead-of'
            : null,
        TYPE:
            substitution.SUBJECT_ID_SUBSTITUTING && type === 'teacher' && substitution.isOld
                ? 'INSTEAD_OF'
                : substitution.TYPE,
    };
};
...
function transformNew(substitution) {
    return {
        ...
        CLASS_IDS_NEW:
            (substitution.CLASS_IDS_NEW && substitution.CLASS_IDS_NEW.length && substitution.CLASS_IDS_NEW) ||
